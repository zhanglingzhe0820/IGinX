...
public class HeuristicPolicy implements IPolicy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HeuristicPolicy.class);
     ...
     * tests
     */
    public Pair<Map<TimeSeriesRange, List<FragmentMeta>>, List<StorageUnitMeta>> generateInitialFragmentsAndStorageUnitsByClients(
    ...
    public void executeReshardAndMigration(
        ...
        double maxLoad = totalHeat / nodeFragmentMap.size() * (1 + config.getHeuristicUnbalanceThreshold());
        Map<FragmentMeta, Long> fragmentTotalLoadMap = new HashMap<>();
        ...
        for (Entry<FragmentMeta, Long> fragmentWriteLoadEntry : fragmentWriteLoadMap.entrySet()) {
            ...
            fragmentTotalLoadMap.put(fragmentWriteLoadEntry.getKey(), load);
        }
        for (Entry<FragmentMeta, Long> fragmentReadLoadEntry : fragmentReadLoadMap.entrySet()) {
            ...
            fragmentTotalLoadMap.put(fragmentReadLoadEntry.getKey(), fragmentTotalLoadMap.getOrDefault(fragmentReadLoadEntry.getKey(), 0L) + load);
        }
        ...
        long minNode = -1;
        long minNodeLoad = Long.MAX_VALUE;
        for (Entry<Long, Long> nodeLoadEntry : nodeLoadMap.entrySet()) {
            if (nodeLoadEntry.getValue() < minNodeLoad) {
                minNodeLoad = nodeLoadEntry.getValue();
                minNode = nodeLoadEntry.getKey();
            }
        }

        double averageFragmentLoad = totalFragmentNum * 1.0 / totalFragmentNum;
        List<MigrationTask> migrationTasks = new ArrayList<>();
        for (Entry<FragmentMeta, Long> fragmentLoadEntry : fragmentTotalLoadMap.entrySet()) {
            long load = fragmentLoadEntry.getValue();
            if (averageFragmentLoad * (1 + config.getHeuristicUnbalanceThreshold()) < load) {
                FragmentMeta fragmentMeta = fragmentLoadEntry.getKey();
                if (fragmentMeta.getMasterStorageUnit().getStorageEngineId() != minNode) {
                    migrationTasks.add(new MigrationTask(fragmentMeta,
                        ...
                        fragmentMetaPointsMap
                            .getOrDefault(fragmentMeta, MigrationTask.RESHARD_MIGRATION_COST), fragmentMeta.getMasterStorageUnit().getStorageEngineId(),
                        minNode,
                        MigrationType.WHOLE));
                }
            }
